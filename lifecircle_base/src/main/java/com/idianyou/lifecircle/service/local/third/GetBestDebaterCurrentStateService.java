...
@Slf4j
@Service
public class GetBestDebaterCurrentStateService extends AbstractGetServiceCurrentStateService {
    ...
    public boolean supports(DataTypeEnum dataTypeEnum) {
        return DataTypeEnum.BEST_DEBATER.equals(dataTypeEnum);
    }
    ...
    public UpdateServiceBizParamDTO getServiceCurrentState(String serviceDataId) {
        ...
        RequestBody formBody = new FormBody.Builder()
                ...
                .add("dataType", DataTypeEnum.BEST_DEBATER.getType().toString())
                .build();
        ...
        try {
            ...
            if (!response.isSuccessful()) {
                log.error("获取[最佳辩手]当前状态数据失败：serviceDataId={}, response={}", serviceDataId, JSON.toJSONString(response));
                return null;
            }
            ...
            if (!"200".equals(body.getString("resultCode"))) {
                log.error("获取[最佳辩手]当前状态数据失败：serviceDataId={}, body={}", serviceDataId, JSON.toJSONString(body));
                return null;
            }
            ...
            log.error("获取[最佳辩手]当前状态数据出错：serviceDataId={}", serviceDataId, e);
        }
